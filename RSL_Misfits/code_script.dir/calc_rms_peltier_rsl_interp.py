...
def calc_RMS_grid(idGridFile):
	...
	for idx_site in range(len(selectedUTKEY)):
	    ...
	    #idx_rslModel = idx_rslModel.astype(int)
            ...
            # Interpolate the model predictions at RSL data time
            f = interpolate.interp1d(rslModelTime,rslModelAtSite)
            rslModelAtSite_interp = f(ageAtSite)
	    ...
	    err[idx_site] = np.mean(((rslDataAtSite - rslModelAtSite_interp) / rslDataError)**2)*Jacobi[idx_site]
...
np.savetxt("ICE6G_rms_Peltier_rsl_NA_ALL_Jacobi_digitized_NA_hudsonbay_interp.txt", rms)
