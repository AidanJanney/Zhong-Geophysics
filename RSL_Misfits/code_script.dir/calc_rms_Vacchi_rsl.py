...
def calc_RMS_grid(idGridFile):
	...
	for idx_site in range(len(selectedUTKEY)):
	    ...
	    #rslDataAtSite[rslDataAtSite==0.0] = 0.5
            ...
            #err[idx_site] =  np.sum(((rslDataAtSite - rslModelAtSite[idx_rslModel])/np.mean(rslDataAtSite)/rslDataError/np.mean(rslDataError))**2)*Jacobi[idx_site]
...
#np.savetxt("ICE6G_rms_Vacchi_edit_norm.txt", rms)
np.savetxt("ICE6G_rms_Vacchi_edit_timeErr.txt", rms)
