...
def calc_RMS_grid(idGridFile):
	...
	for idx_site in range(len(selected_sitename)):
	    ...
	    ageAtSite = age[idx_sitename]
            # Only use RSL data with age < 26.0 kybp
            idx_rsl = ageAtSite <= 26.0
            rslDataAtSite = rslDataAtSite[idx_rsl]
            rslerrAtSite = rslerrAtSite[idx_rsl]
            ageAtSite = ageAtSite[idx_rsl]
...
np.savetxt("ICE6G_rms_farfield_start26.txt", rms)
