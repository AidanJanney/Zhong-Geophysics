...
def calc_rsl_prediction(idGridFile):
	...
	nepochs = 55 #55 for ANU; 50 for ice6g
	caseid = 'grids_compr_lambeck_NA' #'grids_compr_peltier_digitized'#'grids_compr_lambeck_NA' #'grids_compr_lambeck_farfield' #'grids_compr'
	...
	#data_locations = np.loadtxt('RSL_site_peltier_location_selected.txt') 
	#lats = data_locations[:,1]
	#lons = data_locations[:,0]
	#lons[lons < 0] += 360.    #Peltier's longitude range is -180 to 180

	#data_locations = np.loadtxt('RSL_site_location_selected_Lambeck_FarField.txt') 
	data_locations = np.loadtxt('RSL_site_location_selected_Lambeck_NA.txt') 
