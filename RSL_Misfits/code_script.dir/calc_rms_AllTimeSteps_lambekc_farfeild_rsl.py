...
def calc_RMS_grid(idGridFile):
	...
	#err = np.zeros(len(selected_sitename))
        #err = np.zeros((len(selected_sitename), 3))  # Format: Longitude, Latitude, misfit
	...
	for idx_site in range(len(selected_sitename)):
	    ...
	    # Jacobi
            ...
            #err[idx_site,2] = np.mean(((rslDataAtSite - rslModel_interp) / rslDataError)**2)
...
#np.savetxt("ICE6G_rms_Lambeck_farfield_rsl.txt", rms)
...
#np.savetxt("ICE6G_rms_presentday_Lambeck_farfield_594.txt", rms, fmt="%0.2f  %0.2f  %6.3f")
#np.savetxt("ICE6G_m1_rms_alltimesteps_Lambeck_farfield_594.txt", rms, fmt="%0.2f  %0.2f  %6.3f  %6.3f     %6.3f  %6.3f   %6.3f  %6.3f")
np.savetxt("ICE6G_vm5a_rms_alltimesteps_Lambeck_farfield.txt", rms, fmt="%0.2f  %0.2f  %6.3f  %6.3f     %6.3f  %6.3f   %6.3f  %6.3f")
