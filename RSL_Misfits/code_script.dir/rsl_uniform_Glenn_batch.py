...
def calc_eustatic_rsl(idGridFile):
        print(idGridFile)
	...
	ddir1 = './grid' + idGridFile  #'.'#'../check_code/case_incompr_Lambeck1/'#'../check_code/CASE001/'
	ddir='/Users/kkx_pp/Documents/Project_GIA/test_ice6g_kang/ANU/XYZ' #'../Maine_Glenn/4Shijie/XYZ'
	ddir_ocean='/Users/kkx_pp/Documents/Project_GIA/test_ice6g_kang/Maine_Glenn/4Shijie/ocean_anu120p83'  #'../Maine_Glenn/4Shijie/ocean_ice6g120p55'
	...
	if 1:
		...
		np.savetxt(path.join(ddir1, otxt1), rsl,fmt='%14.5e  ')

	if 1:
		otxt3 = 'RSL_eustatic_vm5a_compr.txt' #'RSL_eustatic_Glenn.txt'
		np.savetxt(path.join(ddir1, otxt3), rsl_eustatic,fmt='%14.5e  ')



	if 1:
		otxt2 = 'ice_volume.txt'
		np.savetxt(path.join(ddir1, otxt2), ice_volume,fmt='%14.5e  ')
...
for idx_grid in range(len(idGridFileArr)):
        ...
        # Call the function
        calc_eustatic_rsl(idGridFile=idGridFile)
