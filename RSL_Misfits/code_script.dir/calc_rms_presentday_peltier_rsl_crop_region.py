...
def calc_RMS_grid(idGridFile):
	''' 
        ...
        id_region = 3 #1-NA 2-GreenLand 3-FN 4-Antarctica 0-elsewhere
	...
	err = np.zeros((len(selectedUTKEY), 3)) #lon, lat, misfit
	for idx_site in range(len(selectedUTKEY)):
	    ...
	    #err[idx_site] = np.mean(((rslDataAtSite - rslModelAtSite[idx_rslModel]) / rslDataError)**2)
            ...
            err[idx_site,2] = np.mean(((rslDataAtSite - rslModelAtSite[idx_rslModel]) / rslDataError)**2)
	    # Jacobi
...
#np.savetxt("ICE6G_rms_Peltier_rsl_region_NA.txt", rms)

# Calculate the rms distribution at each rsl sites for the best fit model grids563 for ANU + Lameck's NA rsl dataset
rms = calc_RMS_grid(idGridFile="693")
#np.savetxt("ICE6G_rms_presentday_Peltier_NA.txt", rms, fmt="%0.2f  %0.2f  %6.3f")
np.savetxt("ICE6G_rms_presentday_Peltier_FN.txt", rms, fmt="%0.2f  %0.2f  %6.3f")
